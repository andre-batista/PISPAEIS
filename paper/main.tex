	...
	\section{Computational Experiments}\label{sec:results}
		...
		\subsection{Position Detection Study}\label{sec:results:position}
			...
			\subsubsection{Multiple scatterers}\label{sec:results:position:multiple}
                ...
                Baseado nisto, neste experimento são considerados três espalhadores: a estrela

                % Based on this approach, this experiment considers three scatterers: the star-shaped object from the previous study, a circular scatterer, and a square scatterer. All objects have a contrast of 7 and are positioned at different locations within the DOI to create a challenging multi-target scenario. The goal is to evaluate how well each algorithm can detect the overall position of the scatterer arrangement when individual object geometries cannot be accurately recovered.

                % Fig. \ref{fig:position:multiple:recons} shows the reconstructions obtained by each algorithm for the multiple scatterer scenario. As expected in this high-contrast, small-size configuration, the algorithms reconstruct blob-like structures rather than distinct object geometries. The quality and positioning of these reconstructed features directly influence the centroid calculation and, consequently, the position error measurement.
