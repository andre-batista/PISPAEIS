function [epsilon_r,sigma_eff,dx,dy,mi] = loadphantom2Dset ...
    (filepath,f,epsilon_rb,sigma_b,new_res,slices)
...
%     temp = round(linspace(1,I,slices+2))*dx
    
    epsilon_r = epsilon_r(round(linspace(1,I-10,slices+2)),:,:);
    sigma_eff = sigma_eff(round(linspace(1,I-10,slices+2)),:,:);
    epsilon_r([1,end],:,:) = [];
    sigma_eff([1,end],:,:) = [];
    
    mi = cell(slices,1);
    for i = 1:slices
        mi{i} = find(squeeze(epsilon_r(i,:))==epsilon_rb & squeeze(sigma_eff(i,:))==sigma_b);
    end
