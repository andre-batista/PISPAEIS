import sys
sys.path.insert(1, '../../../eispy2d/library/')

import inputdata as ipt
import casestudy as cst
import richmond as ric
import regularization as reg
import mom_cg_fft as mom
import stopcriteria as stp
import lsm
import osm
import bim
import csi
import som

name = 'star'
filepath = '../../../data/indicatorcomparison/shape/'
resolution = (40, 40)

test = ipt.InputData(import_filename=name + '.ipt',
                     import_filepath=filepath)

method = [
    som.SubspaceBasedOptimizationMethod(stp.StopCriteria(max_iterations=10),
                                        cutoff_index=5, alias='som10'),
    som.SubspaceBasedOptimizationMethod(stp.StopCriteria(max_iterations=20),
                                        cutoff_index=5, alias='som20'),
    som.SubspaceBasedOptimizationMethod(stp.StopCriteria(max_iterations=30),
                                        cutoff_index=5, alias='som30'),
    som.SubspaceBasedOptimizationMethod(stp.StopCriteria(max_iterations=100),
                                        cutoff_index=5, alias='som100')
]

discretization = ric.Richmond(configuration=test.configuration,
                              elements=resolution)

casestudy = cst.CaseStudy(name=name + '.cst',
                           method=method,
                           discretization=discretization,
                           test=test)

casestudy.run(parallelization=cst.PARALLELIZE_METHOD, pre_save=True,
              file_path=filepath)

casestudy.save(save_test=True, file_path=filepath)