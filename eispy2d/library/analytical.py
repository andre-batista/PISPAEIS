...
class Analytical(fwr.ForwardSolver):
    ...
    def __init__(self, contrast=None, radius=None, position=[0, 0]):
        """Create the object.
    ...
    def dielectric_cylinder(self, inputdata, SAVE_INTERN_FIELD=True,
        ...
        if SAVE_MAP or SAVE_INTERN_FIELD:
            # Mesh parameters
        ...
        if SAVE_INTERN_FIELD:
            et = compute_total_field(x, y, a, an, cn, n, kb, kd,
                                     inputdata.configuration.E0, self.position,
                                     thetal)

        if SAVE_MAP:
            # Map of parameters
            epsilon_r, _ = get_map(x, y, a, 
                                   inputdata.configuration.epsilon_rb, 
                                   epsilon_rd)

        # Scatered field
        rho = inputdata.configuration.Ro
        xm = rho*np.cos(thetam) - self.position[1]
        ym = rho*np.sin(thetam) - self.position[0]
...
def compute_total_field(x, y, radius, an, cn, N, wavenumber_b, wavenumber_d,
    ...
    if theta is None:
        ...
        results = Parallel(n_jobs=-1)(delayed(process_s)(s, theta, x, y, N, E0,
                                                         a, kb, kd, an, cn,
                                                         position) for
                                      s in range(S))
...
def compute_scattered_field(xm, ym, an, n, kb, theta, magnitude, position):
    """Summarize the method."""
    ...
    for s in range(S):
        ...
        for j in range(phi.size):
            es[j, s] = E0*np.sum(1j**(-n)*an*hankel2(n, kb*rho[j])
                                 * np.exp(1j*n*phi[j]))* np.exp(-1j*kb*(position[1]*np.cos(theta[s]) + position[0]*np.sin(theta[s])))

    return es


def process_s(s, theta, x, y, N, E0, a, kb, kd, an, cn, position):
    xp, yp = rotate_axis(theta[s], x.reshape(-1), y.reshape(-1))
    rho, phi = cart2pol(xp, yp)
    i = 0
    et_s = np.zeros(x.size, dtype=complex)
    for n in N:
        et_s[rho > a] += (E0*1j**(-n)*(jv(n, kb*rho[rho > a])
                                       + an[i]*hankel2(n, kb*rho[rho > a]))
                          * np.exp(1j*n*phi[rho > a])
                          * np.exp(-1j*kb*(position[1]*np.cos(theta[s]) 
                                           + position[0]*np.sin(theta[s]))))

        et_s[rho <= a] += (E0*1j**(-n)*cn[i]*jv(n, kd*rho[rho <= a])
                           * np.exp(1j*n*phi[rho <= a])
                           * np.exp(-1j*kb*(position[1]*np.cos(theta[s]) 
                                           + position[0]*np.sin(theta[s]))))
        i += 1
