...
class BornIterativeMethod(dtm.Deterministic):
    ...
    def solve(self, inputdata, discretization, print_info=True,
        ...
        if initial_guess is None:
            ...
            solution = self._get_solution(contrast, inputdata.configuration)
            self.forward.solve(solution, noise=0., PRINT_INFO=False,
                               SAVE_INTERN_FIELD=True)
            total_field = solution.total_field.copy()
    ...
    def _get_solution(self, contrast_image, configuration):
        if not configuration.good_conductor:
            rel_permittivity = cfg.get_relative_permittivity(
                contrast_image, configuration.epsilon_rb
            )
        else:
        ...
        if not configuration.perfect_dielectric:
            conductivity = cfg.get_conductivity(
                contrast_image, 2*np.pi*configuration.f,
                configuration.epsilon_rb,
                configuration.sigma_b
            )
        else:
            conductivity = None

        solution = ipt.InputData(
            name='aux', configuration=configuration,
