...
class Benchmark(exp.Experiment):
    ...
    def run(self, parallelization=None, pre_save=False, continue_runs=False,
            file_path=''):
        ...
        if self._single_method and self._single_testset:
            if continue_runs and self.results is not None:
                n0 = len(self.results)
            else:
                n0 = 0
            if parallelization == True:
                ...
                output = (
                    ...
                    (delayed(self.method.solve)(self.testset.test[n],
                                                self.discretization,
                                                print_info=False)
                     for n in range(n0, self.testset.sample_size))
                )
                if continue_runs:
            ...
            elif parallelization == PARALLELIZE_EXECUTIONS:
    ...
    def boxplot(self, indicator, axis=None, testset=None, method=None,
                ...
                notch=False, paired=False, meanline=False, positions=None,
                widths=0.5):
        if self.results is None:
            raise error.MissingAttributesError('Benchmark', 'results')
        if indicator is None or not(type(indicator) is str
        ...
        if self._single_testset and self._single_method:
            ...
            if axis is None:
                ...
                raise error.WrongTypeInput('Benchmark.boxplot', 'axis',
                                           'Axes or Axes-numpy.ndarray',
                                           str(type(axis)))
            elif nfig == 1 and isinstance(axis, np.ndarray):
                raise error.WrongTypeInput('Benchmark.boxplot', 'axis',
                                           'Axes', str(type(axis)))
            elif nfig > 1 and not isinstance(axis, np.ndarray):
                raise error.WrongTypeInput('Benchmark.boxplot', 'axis',
            ...
            if nfig == 1:
                ...
                if not paired:
                    ...
                    for i in range(len(midx)-1):
                        for j in range(i+1, len(midx)):
                            data = (
                                exp.final_value(indicator,
                                                self.results[midx[i], :])
                                - exp.final_value(indicator,
                                                  self.results[midx[j], :])
                            )
                            rst.add_box(data, axis=axis,
                                        labels=[self.method[midx[i]].alias
                                                + '-'
                                                + self.method[midx[j]].alias],
                                        xlabel='Pair of algorithms',
                                        ylabel=rst.indicator_label(indicator),
                                        color=color, title=tit, notch=notch,
                                        fontsize=fontsize, yscale=yscale,
                                        positions=[ipos])
                ...
                for n in range(nfig):
                    if not paired:
                        ...
                        rst.add_box(data, axis=axis[n],
                                    labels=[self.method[m].alias for m in
                                            midx],
                                    xlabel='Algorithms',
                                    ylabel=rst.indicator_label(indicator),
                        ...
                        for i in range(len(midx)-1):
                            for j in range(i+1, len(midx)):
                                ...
                                label = [self.method[midx[i]].alias + '-'
                                         + self.method[midx[j]].alias]
                                rst.add_box(
                                    data, axis=axis[n], labels=label,
                                    xlabel='Pair of algorithms',
                                    ylabel=rst.indicator_label(indicator),
                                    color=color, title=tit, notch=notch,
                                    fontsize=fontsize, yscale=yscale,
            ...
            if nfig == 1:
                ...
                rst.add_box(y, axis=axis,
                            ...
                            color=color, title=tit, notch=notch,
                            fontsize=fontsize, yscale=yscale,
                            meanline=meanline, positions=positions,
                            widths=widths)
            else:
                if title == False or title == None:
                    plotstitles = [None for n in range(nfig)]
    ...
    def normality(self, indicator, axis=None, method=None, testset=None,
                  ...
                  file_path='', file_format='eps', paired=False):
        ...
        if self._single_method and self._single_testset:
            ...
            if method is None:
                ...
                if not paired:
                    ...
                    nfig = int(len(self.method)*(len(self.method)-1)/2)
            elif type(method) is str:
                ...
                if not paired:
                    ...
                    nfig = int(len(method)*(len(method)-1)/2)
            else:
            ...
            if not paired:
                ...
                nfig = int(len(testset)*len(method)*(len(method)-1)/2)
                    ...
                    for i in range(len(method)-1):
                        for j in range(i+1, len(method)):
                            if title is None or (type(title) is bool
                                                 and title is True):
                                tit.append(rst.TITLES[ind] + ', '
                                           + self.method[method[i]].alias + '-'
                                           + self.method[method[j]].alias)
                            data = (
                                exp.final_value(ind,
                                                self.results[method[i], :])
                                - exp.final_value(ind,
                                                  self.results[method[j], :])
                            )
                            _ = sts.normalitiyplot(data, axes=ax[n],
                                                   title=tit[n],
                                                   fontsize=fontsize)
                            n += 1
            elif self._single_method and not self._single_testset:
    ...
    def confint(self, indicator, method=None, testset=None, print_info=True,
                ...
                confidence_level=.95, group='methods', paired=False):
        if indicator is None:
        ...
        if self._single_method and self._single_testset:
                    ...
                    for i in range(len(method)-1):
                        for j in range(i+1, len(method)):
                            ...
                            message += self._print_confint(
                                self.method[method[i]].alias + '-'
                                + self.method[method[j]].alias, output,
                                confidence_level
                            )
                            if not print_info and not normality:
                                name = (ind + ', '
            ...
            if plot_figure:
                ...
                if not paired:
                    ...
                    for i in range(len(method)-1):
                        for j in range(i+1, len(method)):
                            ylabel.append(self.method[method[i]].alias + '-'
                                          + self.method[method[j]].alias)
                if type(title) is bool and title is False:
                    tit = [None for n in range(NFIG)]
                elif title is None or (type(title) is bool and title is True):
