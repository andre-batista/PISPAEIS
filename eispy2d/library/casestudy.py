...
class CaseStudy(exp.Experiment):
    ...
    def reconstruction(self, image=CONTRAST, axis=None, method=None,
                       ...
                       include_true=False, mode=ALL_EXECUTIONS, vmin=None,
                       vmax=None):
        ...
        if self._single_method or type(method) is int or type(method) is str or (type(method) is list and len(method) == 1):
            ...
            for i in ind:
                ...
                if ((self._single_method
                     and isinstance(self.method, dtm.Deterministic)) 
                        or (not self._single_method
                            and isinstance(self.method[midx],
                                           dtm.Deterministic))):
                    if isinstance(getattr(self.results[midx], i), np.ndarray):
                        y = getattr(self.results[midx], i)
                    else:
                        y = np.array(getattr(self.results[midx], i))
                    ...
                    if self._single_method:
                        y = np.array(getattr(self.results, i))
                    else:
                        y = np.array(getattr(self.results[midx], i))
            ...
            for i in ind:
                ...
                elif type(color) is list and len(color) < len(midx):
                    raise error.WrongValueInput('CaseStudy.convergence',
                                                'color', 'str-list of size %d'
                                                % len(midx), 'str-list of size'
                                                % len(color))
                elif type(color) is list:
                    cols = color.copy()
                else:
                    raise error.WrongTypeInput('CaseStudy.convergence', 'color',
                                           'None or str-list',
                                           str(type(color)))
                icol = 0
                for m in midx:                    
