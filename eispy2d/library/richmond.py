...
class Richmond(clc.Collocation):
    ...
    def contrast_image(self, coefficients, resolution):
        ...
        elif np.prod(coefficients.shape) < np.prod(resolution):
            x, y = cfg.get_coordinates_ddomain(
                configuration=self.configuration, resolution=self.elements
            )
            xp, yp = cfg.get_coordinates_ddomain(
                configuration=self.configuration, resolution=resolution
            )
            if coefficients.ndim == 1:
                f = NearestNDInterpolator((x.flatten(), y.flatten()), coefficients)
            elif (coefficients.ndim == 2
                    and np.prod(coefficients.shape) == np.prod(x.shape)):
                f = NearestNDInterpolator((x.flatten(), y.flatten()),
                                          coefficients.flatten())
            elif (coefficients.ndim == 2
                    and np.prod(coefficients.shape) == np.prod(x.shape)**2):
                f = NearestNDInterpolator((x.flatten(), y.flatten()),
                                          np.diagonal(coefficients).flatten())
            return f(xp, yp)   
        else:
            if coefficients.ndim != 2:
                error.Error("The 'coefficients' argument must be 2D array.")
            x, y = cfg.get_coordinates_ddomain(
                configuration=self.configuration,
                resolution=coefficients.shape
            )
            xp, yp = cfg.get_coordinates_ddomain(
                configuration=self.configuration, resolution=resolution
            )
